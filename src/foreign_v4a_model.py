"""Foreign player prediction model v4a — Student-t likelihood.

Based on v2 with one key change:
  - Normal likelihood → Student-t for robustness to outliers
  - Estimated nu (degrees of freedom) parameter

All other features identical to v2 (7 improvements over v1).

Usage:
  python src/foreign_v4a_model.py --draws 2000 --warmup 1000
  python src/foreign_v4a_model.py --loo-cv
  python src/foreign_v4a_model.py --expanding-cv
"""
...
def loo_cv_hitters(data: list[dict], draws: int = 2000, warmup: int = 1000) -> dict:
    ...
    from cmdstanpy import CmdStanModel
    model = CmdStanModel(stan_file=str(MODELS_DIR / "hitter_foreign_v4a.stan"))
...
def loo_cv_pitchers(data: list[dict], draws: int = 2000, warmup: int = 1000) -> dict:
    ...
    from cmdstanpy import CmdStanModel
    model = CmdStanModel(stan_file=str(MODELS_DIR / "pitcher_foreign_v4a.stan"))
...
def expanding_cv_hitters(data: list[dict], draws: int = 2000, warmup: int = 1000) -> dict:
    ...
    for test_year in years:
        ...
        mae_v4a = np.mean([abs(p["actual"] - p["pred_v4a"]) for p in preds])
        ...
        results_by_year[test_year] = {
            "n": len(preds),
            "mae_v4a": float(mae_v4a),
            "mae_baseline": float(mae_bl),
            "improvement": float((mae_bl - mae_v4a) / mae_bl * 100),
            "coverage_80": float(cov),
        }
        print(f"  Year {test_year}: n={len(preds)} MAE_v4a={mae_v4a:.4f} MAE_bl={mae_bl:.4f} "
              f"({(mae_bl-mae_v4a)/mae_bl*100:+.1f}%) cov={cov:.1%}")
...
def _summarize_cv(predictions: list[dict], stat_name: str) -> dict:
    ...
    preds_v4a = np.array([p["pred_v4a"] for p in predictions])
    preds_bl = np.array([p["pred_baseline"] for p in predictions])

    mae_v4a = float(np.mean(np.abs(actuals - preds_v4a)))
    ...
    improvement = (mae_bl - mae_v4a) / mae_bl * 100
...
def save_results(hitter_params: dict, pitcher_params: dict,
    ...
    # Posterior parameters (for foreign_bayes.py v4a)
    params_out = {
        "version": "v4a",
    ...
    with open(output_dir / "foreign_v4a_posterior.json", "w") as f:
        json.dump(params_out, f, indent=2, ensure_ascii=False)
    print(f"Saved: {output_dir / 'foreign_v4a_posterior.json'}")

    # Backtest results
    with open(output_dir / "foreign_v4a_backtest.json", "w") as f:
    ...
    print(f"Saved: {output_dir / 'foreign_v4a_backtest.json'}")
    ...
    for ptype in ["hitter", "pitcher"]:
        ...
        csv_path = output_dir / f"foreign_v4a_{ptype}_loo_predictions.csv"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Foreign player v4a model (Student-t)")
    ...
    print("=" * 60)
    print("Foreign Player Prediction Model v4a (Student-t)")
    print("=" * 60)
    ...
    fit_h = fit_model("hitter_foreign_v4a.stan", stan_h,
    ...
    fit_p = fit_model("pitcher_foreign_v4a.stan", stan_p,
                      draws=args.draws, warmup=args.warmup)
